"""
Train ResNetBiLSTM — 40 epochs, all 32 subjects.

Usage
-----
    python src/training/train_resnet_bilstm.py

Output
------
    models/resnet_bilstm_v1.0.pth
"""
...
from training.helpers import EarlyStopping


def main() -> None:
    print("🚀 Loading all subjects for ResNetBiLSTM...")
    ...
    cw_tensor = torch.tensor(class_w, dtype=torch.float).to(device)

    X_scaled      = StandardScaler().fit_transform(X_raw)
    X_win, y_win  = create_windows(X_scaled, y_mapped, window_size=50, step_size=25)
    print(f"🪟 Windows: {X_win.shape} | Classes: {num_classes}")
    ...
    model         = ResNetBiLSTM(len(feature_cols), num_classes).to(device)
    ...
    checkpoint_path  = os.path.join(MODELS_DIR, "resnet_bilstm_v1.0.pth")
    print(f"\n🔥 Training ResNetBiLSTM ({epochs} epochs)...")
    t0 = time.time()
    ...
    for epoch in range(epochs):
        ...
        print(f"[E{epoch+1:02d}/{epochs}] Train:{100.*correct/total:5.2f}% | Val:{val_acc:5.2f}% | VLoss:{v_l_avg:.3f} {marker}", flush=True)
