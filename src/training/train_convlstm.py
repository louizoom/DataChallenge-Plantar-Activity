"""
Train ConvLSTM — 35 epochs, subjects S01–S05.
...
def main() -> None:
    print("🚀 Loading S01–S05 for ConvLSTM...")
    ...
    model     = ConvLSTM(num_features, num_classes).to(device)
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), lr=0.002, weight_decay=1e-4)
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode="max", factor=0.5, patience=3)

    epochs, best_acc = 35, 0.0
    checkpoint_path  = os.path.join(MODELS_DIR, "convlstm_v1.0.pth")
    print(f"\n🔥 Training ConvLSTM ({epochs} epochs)...")
    t0 = time.time()
    ...
    for epoch in range(epochs):
        ...
        print(f"[{epoch+1:02d}/{epochs}] Train:{100.*correct/total:5.2f}% | Val:{val_acc:5.2f}% {marker}")
