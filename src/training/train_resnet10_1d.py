"""
Train ResNet10_1D — Champion Model (78.23 % validation accuracy).
...
from models import ResNet10_1D


def main() -> None:
    print("🚀 Loading all subjects for ResNet10_1D (Champion)...")
    ...
    model = ResNet10_1D(num_features, num_classes).to(device)
    ...
    epochs, best_acc = 50, 0.0
    checkpoint_path = os.path.join(MODELS_DIR, "resnet10_1d_v1.0.pth")
    print(f"\n🔥 Training ResNet10_1D ({epochs} epochs)...")
    t0 = time.time()
